...
date: 2026-03-29
slug: 'ai-optimized-means-ai-ready'
draft: false
featured: false
cover: './ai-optimized-means-ai-ready-hero.svg'
cardImage: './ai-optimized-means-ai-ready-hero.svg'
coverAlt: 'Layered AI constitution diagram showing CLAUDE.md as the top-level layer, with engineering.md, workflow.md, and stack.md cascading beneath it'
...
`CLAUDE.md` is the always-on context file Claude Code reads at the start of every session. It's the Claude Code counterpart of the cross-tool `AGENTS.md` — and both encode the same source of truth, the AI Assistant Contract in `docs/ai-context/INDEX.md`.
...
**Progressive disclosure.** The full file is under 130 lines. For detailed conventions — the full component pattern spec, the full ADR set — it references deeper docs by file path. Claude Code loads those on demand, keeping session context lean.
...
The same `client:load` rule that appears in `CLAUDE.md` as rule #2 shows up in `AGENTS.md` (the file Cursor, Copilot, Codex CLI and friends read natively) like this:
...
Both `CLAUDE.md` and `AGENTS.md` are derived from the AI Assistant Contract in `docs/ai-context/INDEX.md`. Update your conventions there, and every AI tool picks them up. See `docs/ai-context/ai-rules-setup.md` for the full multi-tool setup.
</Callout>
...
- **Design tokens** — no hardcoded colors (`bg-white`, `#hex`, `rgb()`), must use semantic tokens (`text-foreground`)
...
<article
  class:list={["rounded-lg border border-border", className]}
  {...rest}
>
...
├── AGENTS.md                         # Cross-tool entry point (Cursor, Copilot, Codex CLI, …)
└── .claude/
...
Every AI tool points to the same project conventions through its own entry point. For the full setup guide covering Claude Code, Cursor, Copilot, and the other `AGENTS.md`-native tools, see `docs/ai-context/ai-rules-setup.md`.
</Callout>
