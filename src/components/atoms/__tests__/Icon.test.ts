...
const renderIcon = (props: Record<string, unknown>, slot?: string) =>
  render(Icon, props, slot ? { default: slot } : {});

describe("Icon (atom)", () => {
  describe("rendering", () => {
    it("renders an <svg> element", async () => {
      const html = await renderIcon({ name: "github" });
    ...
    it("applies a custom class instead of the default", async () => {
      const html = await renderIcon({ name: "github", class: "w-8 h-8 text-primary-600" });
  ...
  describe("accessibility", () => {
    ...
    it("exposes an accessible name via aria-label + role=img", async () => {
      const html = await renderIcon({ name: "github", ariaLabel: "GitHub" });
      expect(html).toContain('aria-label="GitHub"');
      expect(html).toContain('role="img"');
    });

    it("hides from AT when decorative=true even if a label is provided", async () => {
      const html = await renderIcon({ name: "github", ariaLabel: "ignored", decorative: true });
  ...
  describe("custom slot", () => {
    it("renders slot content when name=custom", async () => {
      const html = await renderIcon({ name: "custom" }, '<path d="M0 0h24v24H0z" />');
